...
/* Eklenti PurposeLess tarafindan Turkceye cevirilmistir.
**                              ===== CSDuragi.com =====
*/

//#define CUSTOM_MENU			// 3 Secenekli Menu | 1- Oyuna Gir 6- Izleyici 0- Cikis |
#define TIME_CHANGE	10		// Kac saniye icinde takim degisebilir
#define PLAYER_DIFF	2		// Takimlar arasindaki fark ( n takiminda daha fazla kisi var ise, bu takima giremezsin)
...
public CMD_ChooseTeam(id)
{
	...
	if(TIME_CHANGE && get_member(id, m_iTeam))
	{
		...
		{
			client_print_color(id, 0, "^3%d ^4saniye icinde ^3takim degistirebilirsiniz^1.", floatround(fNextChoose - fCurTime));
			return HC_SUPERCEDE;
		}
	}
	return HC_CONTINUE;
...
public fwdShowVGUIMenu(const id, VGUIMenu:menuType, const bitsSlots, szOldMenu[])
{
	if(menuType == VGUI_Menu_Team)
	{
		...
		SetHookChainArg(4, ATYPE_STRING, "\yAna Menu:^n^n\y1. \rOyuna Gir^n\y6. \wIzleyici^n^n\y0. \wCikis");
#else
		new iTeamTT, iTeamCT; CalculateTeamNum(iTeamTT, iTeamCT);
		new TeamName:team = get_member(id, m_iTeam);
		new szMenu[192], iKeys = MENU_KEY_0;
		new iLen = formatex(szMenu, charsmax(szMenu), "\yTakiminizi Secin:^n^n");
		if((iTeamTT - iTeamCT) >= PLAYER_DIFF || team == TEAM_TERRORIST)
			iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y1. \dTerrorist [\r%d\w]^n", iTeamTT);
		else
		{
			iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y1. \wTerrorist [\r%d\w]^n", iTeamTT);
			iKeys |= MENU_KEY_1;
		}
		...
		iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y5. \yOtomatik Sec^n");
		iKeys |= MENU_KEY_5;
		
		if(team == TEAM_SPECTATOR)
			iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y6. \rIzleyici^n^n^n");
		else
		{
			iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y6. \rIzleyici^n^n^n");
			iKeys |= MENU_KEY_6;
		...
		formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y0. \wCikis");
	...
	else if(menuType == VGUI_Menu_Class_T)
	{
		...
		SetHookChainArg(4, ATYPE_STRING, "\yKarakterini Sec:^n^n\r1. \wPhoenix Connexion^n\r2. \wElite Crew^n\r3. \wArctic Avengers^n\r4. \wGuerilla Warface^n^n\r5. \yOtomatik Sec^n^n^n\r0. \wCikis");
	}
	else if(menuType ==  VGUI_Menu_Class_CT)
	{
		SetHookChainArg(3, ATYPE_INTEGER, MENU_KEY_0 | MENU_KEY_1 | MENU_KEY_2 | MENU_KEY_3 | MENU_KEY_4 | MENU_KEY_5);
		SetHookChainArg(4, ATYPE_STRING,"\yKarakterini Sec:^n^n\r1. \wSeal Team 6^n\r2. \wGSG-9^n\r3. \wSAS^n\r4. \wGIGN^n^n\r5. \yOtomatik Sec^n^n^n\r0. \wCikis");
	}
	return HC_CONTINUE;
}
